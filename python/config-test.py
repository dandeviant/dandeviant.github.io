...
vlan_cctv = input("CCTV VLAN:")
vlan_qua = ""
vlan_qua = input("Quarantine VLAN (blank for non-dot1x): ")
...
for x in range(vlan_count):
    if vlan_list[x] == "trunk":
        ...
        print(" exit")
        print("! ")
    elif vlan_list[x] == vlan_cctv:
        print("interface GigabitEthernet1/0/%s" % (x+1))
        print(" description #CCTV")
        print(" switchport access vlan %s" % (vlan_list[x]))
        print("! ")
    elif vlan_list[x] == vlan_qua:
        print("interface GigabitEthernet1/0/%s" % (x+1))
        print(" switchport access vlan %s" % (vlan_native))
        print("! ")
    else:
        print("interface GigabitEthernet1/0/%s" % (x+1))
        print(" switchport access vlan %s" % (vlan_list[x]))
